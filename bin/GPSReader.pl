...
Readonly my $DEFAULT_GPS_PORT => shift @ARGV || '/dev/ttyS0';
Readonly my $DEFAULT_GPS_BAUD => shift @ARGV || 38_400;
...
sub emit_message {
        ...
        && defined $NMEADATA{speed_in_kph} )
    {
        ...
        my $velocity = $NMEADATA{speed_in_kph}+0;
...
sub open_port {
    ...
    my $portobj = Device::SerialPort->new($port) or die "do you have the wrong tty? ($port)($baud)($?)($!)";
